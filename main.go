...
// ... (в начале файла, в функции escapeMarkdownV2)
func escapeMarkdownV2(text string) string {
	...
	// Пользовательская логика показала, что '!' также может требовать экранирования.
...
func main() {
	...
	for update := range updates {
		...
		case cmdStart:
			// originalText теперь будет экранироваться, поэтому форматирование ссылки не сработает.
			// Изменим текст, чтобы он содержал URL как обычный текст или удалим его.
			...
			escapedText := escapeMarkdownV2(originalText) // <-- Теперь экранируем
		...
		case cmdHelp:
			// originalText теперь будет экранироваться, поэтому форматирование ссылки не сработает.
			// Вариант 1: Удалить ссылку, оставить только упоминание
